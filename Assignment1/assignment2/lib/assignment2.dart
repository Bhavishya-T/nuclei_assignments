...
class UserUtils {
  static void addUser() {
    try {
      ...
      Validators.ensurePositive(age);
      print('Enter Address :');
      String address = stdin.readLineSync()!;
      print('Enter Roll Number :');
      int rollNumber = int.parse(stdin.readLineSync()!);
      Validators.ensurePositive(rollNumber);
      Validators.rollNumberValidator(rollNumber);
      print('Enter courses you want to enroll in (Choose 4 - A,B,C,D,E,F) :');
      String? courses = stdin.readLineSync();
      List<String> enrolledCourses = Validators.acceptCourses(courses);
  ...
  static void displayUser() {
    try {
      ...
      print(
          "On what basis should the users be sort, choose option from below :");
      print('''
  1. Name
  2. Roll Number
  3. Age
...
--------------------------------------------------------------------------------------------------------------------------------- 
  ...
  static void deleteUser() {
    try {
      ...
      int rollNumber = int.parse(stdin.readLineSync()!);
      Validators.checkRollNumber(rollNumber);
